...
def format_generated_answer(text):
    prompt = f"Format the text by adding not too many emojis, keep it concise to the provided information, no need to provide any further information since this is a report for me: {text}"
...
class ActionGenerateTrendReport(Action):
    ...
    def run(
        ...
        formatted_report = format_generated_answer(report)

        dispatcher.utter_message(response="utter_analyzing_trends")
        dispatcher.utter_message(text=formatted_report)
        return [Restarted()]
...
class ActionAskStep(Action):
    ...
    def run(
        ...
        text = (f"Can you provide me the step for the {report_type} report?\n"
                f"Currently I can help you with these kinds of intervals:\n")
        buttons = []
...
class ValidateProfitRevenueForm(FormValidationAction):
    ...
    async def required_slots(
        ...
        if end_date == "Now" and start_date is None:
            try:
                updated_slots.remove("start_date")
            except ValueError:
                pass
            ...
            if report_amount == "All":
                ...
                if len(result) != 0:
                    ...
                    for row in result:
                        ...
                        period_start_parsed = period_start.strftime(date_format)
                        period_end_parsed = period_end.strftime(date_format)
                        ...
                        report += f"{period_start_parsed} to {period_end_parsed}: {total}\n"
